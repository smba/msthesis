\section{Variability Model Synthesis}
The idea of designing and developing configurable software systems is driven by
the separation of different concerns, or features of a software system. A
software system which is configurable is designed to be assembled at
compile-time with respect to its configuration, or to behave according to a
configuration read at load-time. Moreover, for both types of variability there
exist various ways of realization. A complete survey of means to implement
configurable software systems would likely exceed the scope of this section,
yet we intend to present in this section which shapes of variability are
meaningful and how they can be assessed. The next two subsections address on
the one hand, how variable program parts can be extracted, and, on the other
hand, how variability models itself can be recovered from the code base. While
the first objective yields representations, which can be a basis for numerous
variability-aware analyses of the program, the second yields useful knowledge
on how to actually configure a software system. The third subsection discusses
the transformation of variability constraints to a hierarchical feature diagram,
and the last subsection discusses practical remarks on synthesizing a
variability model.

\subsection{Untangling Configurable Software Systems}
With the phenomenon of an infeasible number of configurations, naive analyses of
configurable systems are not trivial since static analysis can only assess one
variant at a time, and dynamic analyses only follow one execution path.
Although some approaches to implement variability provide modularization to
some extent, their representation specifies under which
configurations a code fragment is refined and compiled, or executed. In
contrast to that, we know little about the resulting variants without compiling
every single variant possible, as well as about every execution path without
explicitly marking alternatives. In this subsection, however, we discuss among
others techniques to obtain information about every variant possible without a
single assessment.

\cite{kastner_variability-aware_2011} have proposed a framework,
\emph{TypeChef}, to enable the construction of variability-aware parsers. A variability-aware parser, like
...
nesting further nodes. In that way, the variable and invariant program segments
can be untangled.
...
Besides variability-aware parsing, \cite{nguyen_building_2014} have applied symbolic
execution to unwind variability for PHP Web applications. Web applications are
staged, i.e., even though it can be configured at load-time, the applications
is as well variable with respect to input received at run-time. For instance,
...
\label{sec:feature_model_synthesis} A variability model as an abstraction of functionality of a software system is
...
focuses on the extraction of file presence conditions from build files using
symbolic execution. \cite{al-kofahi_escaping_2016}
}
...
\subsection{Reverse Engineering Feature Diagrams} \label{sec:feature_hierarchy}
